										...
										<table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
											<thead>
												<tr>
													<td colspan="2">
														<?php if ($row['cpu_id'] > 0) {
            echo '<a  href="' . $_SERVER['PHP_SELF'] . '?action=clear&regno=' . base64_encode($row['regno']) . '" id="clear" onClick="return confirm(\'Are you sure about this\')"  class="btn btn-danger" name=""><i class="icon-trash icon-large"></i> Clear Student Field Of Study</a>';
        } else {
            echo '<a   href="' . $_SERVER['PHP_SELF'] . '?action=assign&regno=' . base64_encode($row['regno']) . '" id="allocate" onClick="return confirm(\'Are you sure about this\')"  class="btn btn-success" name=""><i class="icon-signin icon-large"></i> Assign Student To Field</a>';
        }
        ?>
													</td>
												</tr>
...
if (isset($_GET['action'], $_GET['regno']) && ($_GET['action'] == 'clear' || $_GET['action'] == 'assign')) {
    ...
    if ($row < 1) {
        ...
        if ($_GET['action'] == 'clear') {
            ...
            $row2 = getAllSupervisor(0);

            if ($row || $row2) {
                ?>
																<select required name="supervisor" id="" style="width:100%">
																	<option value="">Choose Supervisor</option>
																	<optgroup label='Supervisors without fields'>
<?php
if ($row2) {
                    while ($result = $row2->fetch_assoc()) {
                        $display = $result['bio'] . " has a max. of " . $result['myMax'];
                        $id = $result['id'];
                ...
                ?>
																	...
																	<optgroup label='Supervisors with fields'>
																	<?php while ($result = $row->fetch_assoc()) {
                    // $display = $result['bio'] . " Chose " . $result['no'] . " field" . (($result['no'] > 1) ? 's' : '') . " and has a max. of " . $result['myMax'] . ", " . $result['current'] . " out of " . $result['expected'] . " assigned";
                    $display = $result['bio'] . " Chose " . $result['no'] . " field" . (($result['no'] > 1) ? 's' : '') . " Has " . $result['current'] . " assigned out of " . $result['myMax'];
            ...
            ?></td>
													...
													<tr>
														<td colspan="2" class='alert alert-info'>Request will only be sent to supervisors who already used up his/her maximum quota.
															<hr />If request is sent to supervisor who is yet to use his/her quota, request will be auto-granted.</td>
													</tr>
													<tr>
														<td colspan="2">
															<?php
if ($row || $row2) {?>
    ...
    $getAssign = getAllStudentInAssign(1);
    ?>
						<div id="block_bg" class="block">
							<div class="navbar navbar-inner block-header">
								<div class="muted pull-left">All Special Requests Sent</div>
							</div>
							<div class="block-content collapse in">
								<div class="span12">
										...
										<tbody>
											...
											<?php while ($row = $getAssign->fetch_assoc()) {?>
												<tr>
													...
													<td><?php $get = (getFieldById($row['field_id']));
        echo $get['name'];?></td>
													<td><?php echo (getSupervisorNameById($row['supervisor_id'])); ?></td>
													<td><?php
